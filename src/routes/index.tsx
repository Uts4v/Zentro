...
// punches_to_free here is just a placeholder — it gets overwritten with the
// merchant's real config value as soon as loadPunchCard() runs.
...
function Loyalty() {
  ...
  // punches_to_free is always read fresh from merchant_profiles here, rather
  // than trusting the (possibly stale) copy on the punch_cards row — this is
  // what fixes the "merchant changes punch count but customer page doesn't
  // update" bug.
  const loadPunchCard = useCallback(async () => {
    ...
    setPunchError(null);
    try {
      const [cardRes, merchantRes] = await Promise.all([
        customerApi.getPunchCard(selectedMerchantId),
        supabase
      ...
      const punchesToFree = merchantRes.data?.punches_to_free ?? 5;

      if (cardRes) {
        setPunchCard({
          ...cardRes,
          punch_count: cardRes.punch_count ?? 0,
          punches_to_free: punchesToFree,
          free_reward_available: cardRes.free_reward_available ?? false,
        });
      } else {
